...
def htmC(c,a_grid,b_grid): # example of adding a het output, gets saved as steady state and agg is returned  
    
    b_grid2=1*(b_grid[np.newaxis,:,np.newaxis]<0.33) # htm flag based on calibration of Y=1
    chtm=(c)*b_grid2
    shrhtm=(c/c)*b_grid2
...
@het(exogenous='Pi', policy=['b', 'a'], backward=['Vb', 'Va'],
     hetinputs=[marginal_cost_grid], hetoutputs=[adjustment_costs,htmC], backward_init=hh_init)  
